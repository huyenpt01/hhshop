...
<body>
    ...
    <main>
        <div class="card-body">
            ...
            // }
            ?> -->
            <h3>Nhập tên sản phẩm muốn tìm</h3>
        ...
        <!-- </div> -->
            ...
            <table class="table">
                ...
                if (isset($_POST['hihi'])) {
                    ...
                    if(empty($title)){
                        echo '<script language="javascript">alert("Vui lòng nhập tên sản phẩm!");
        
                    </script>';
                    }
                    else{
                    
                    $sql = "SELECT product.id_pr,product.title,
                    product.thumbnail,product.price_old,product.price,
                    categroy.name category_name 
                    from product,categroy 
                    WHERE product.id_category=categroy.id_cate  and product.title like '$title%' " ;
                   ...
                   foreach ($result as $row) {
                    ...
                    foreach ($result as $row) {
                        ...
                        <tr>
                            ...
                            <td>
                                <img src="../uploads/'.$row['thumbnail'].'" width="80px">

                            </td>

                            <td>'.number_format($row['price_old'], 0, ",", ".").' VND</td>
        ...
        <h3>Danh sách tất cả sản phẩm</h3>
            ...
            <table class="table table-hover table-bordered">
                ...
                <tbody>
                    <?php
                    $item_per_page = !empty($_GET['per_page']) ? $_GET['per_page'] : 5; // so sp trong 1 trang
                    $current_page = !empty($_GET['page']) ? $_GET['page'] : 1;
                    $offset = ($current_page - 1) * $item_per_page;
                    $product = mysqli_query($connect, "select product.id_pr,product.title,
                    product.thumbnail,product.price_old,product.price,
                    categroy.name category_name 
                    from product 
                    left join categroy on product.id_category=categroy.id_cate 
                    ORDER BY id_pr DESC "); //limit " . $item_per_page . " offset  " . $offset . "
                    // $totalRecords = mysqli_query($connect, "select * from product");
                    // $totalRecords = $totalRecords->num_rows; // tong sp
                    // $totalPages = ceil($totalRecords / $item_per_page); // tong so trang
                    $no = 1;
